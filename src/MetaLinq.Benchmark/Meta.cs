...
static class Meta {
    ...
    public static int Select_Sum(TestData[] testData) => testData.Select(static x => x.Value).Sum();
    public static int Select_Min(TestData[] testData) => testData.Select(static x => x.Value).Min();
    public static int? Select_SumN(TestData[] testData) => testData.Select(static x => x.Value % 2 == 0 ? null : (int?)x.Value).Sum();
    public static int? Select_MinN(TestData[] testData) => testData.Select(static x => x.Value % 2 == 0 ? null : (int?)x.Value).Min();
